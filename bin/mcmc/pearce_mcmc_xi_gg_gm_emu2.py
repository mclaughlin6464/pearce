...
y_gg = np.loadtxt('xi_gg_true_mean_%d%d.npy'%(boxno, realization))
y_gg = y_gg[-emu_gg.n_bins:]

y_gm = np.loadtxt('xi_gm_true_mean_%d%d.npy'%(boxno, realization))
...
chain_fname = path.join(savedir, '%d_walkers_%d_steps_xi_gg_gm_m%d_%d_rmin_emu2.npy'%(nwalkers, nsteps, boxno, realization))
