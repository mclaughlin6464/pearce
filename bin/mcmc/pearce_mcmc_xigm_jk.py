...
cat.populate(em_params)
y10, cov10 = cat.calc_xi(r_bins, do_jackknife=True, jk_args = {'n_rands':50, 'n_sub':5})
...
np.savetxt('xi_gm_true_jk.npy', y)
#np.savetxt('xi_gg_cov_true_jk.npy', cov)

scov = np.loadtxt('xigm_scov_log.npy')

# TODO removed jackknife
cov+=scov # add the emu covariance

cov+=shot_cov # 

np.savetxt('xi_gm_cov_true_all.npy', cov)
...
chain_fname = path.join(savedir, '%d_walkers_%d_steps_chain_cosmo_zheng_xigm_jk_meanv2.npy'%(nwalkers, nsteps))
