...
for i in xrange(50):
    ...
    xi_vals.append(cat.calc_xi_gm(r_bins))

# TODO need a way to get a measurement cov for the shams
xi_vals = np.array(xi_vals)

cat.populate(em_params)
yjk, cov = cat.calc_xi_gm(r_bins, do_jackknife=True, jk_args = {'n_rands':10, 'n_sub':5})
...
np.savetxt('xi_gm_true_mean_%d%d.npy'%(boxno, realization), y)
np.savetxt('xi_gm_true_jk_%d%d.npy'%(boxno, realization), yjk)

np.savetxt('xi_gm_cov_true_jk_%d%d.npy'%(boxno, realization), cov)

#scov = np.loadtxt('xigg_scov_log.npy')

np.savetxt('xi_gm_shot_cov_true_%d%d.npy'%(boxno, realization), shot_cov)
...
with open('cosmo_gm_param_dict_%d%d.pkl'%(boxno, realization), 'w') as f:
