...
def compute_RR_hack(cat, rbins, n_rands= 5, n_sub = 5, n_split=50, n_cores = 16):
...
#pos_m = return_xyz_formatted_array(x_m, y_m, z_m, period=cat.Lbox)
    ...
    #np.save('/scratch/users/swmclau2/randoms_gm.npy', randoms)
    #print randoms.shape
    split_randoms = np.array_split(randoms, n_split, axis = 0)
    #RR = np.zeros((n_sub**3+1, rbins.shape[0]-1))
    start_idx = 2493#2258#2031#1797#1562#1330#880 
    RR = np.load('RR_%d.npy'%start_idx)
    print len(list(product(split_randoms, split_randoms)))
    for i, (A, B) in enumerate(product(split_randoms, split_randoms)):
        if i< start_idx:
