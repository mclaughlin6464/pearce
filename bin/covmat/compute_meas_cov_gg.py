...
def compute_full_jk(cat, rbins, rand_scalecut = 1.0 ,  n_rands= [10, 5, 5], n_sub = 3, RR=RR):
...
#for rb, nr in zip([rbins_large], n_rands): #
    # nr removed
    np.random.seed(0)
    randoms = np.random.random((pos_g.shape[0]*n_rands[0],3)) * cat.Lbox / cat.h  # Solution to NaNs: Just fuck me up with randoms
    out = tpcf_subregions(pos_g / cat.h, randoms, rbins, sample2 = None, period=cat.Lbox / cat.h,
                          num_threads=n_cores, Nsub=n_sub, estimator='Landy-Szalay',\
                          do_auto1 = True, do_cross = False, do_auto2 = False)
    
    xi_all = np.hstack(out)

    cov = np.cov(xi_all.T, bias = True)*(n_sub-1.0)
...
for boxno in xrange(7):
    for realization in xrange(5):
        ...
        for hod_idx, hod_params in enumerate(hod_dicts):
            ...
            print boxno, hod_idx
            mat = compute_full_jk(cat, r_bins, RR=RR_gg)
            cov_mats[boxno, hod_idx, realization] = mat
 

            # TODO do the reduction here
            np.save('testbox_covmats_gg.npy', cov_mats)
# In[ ]:
