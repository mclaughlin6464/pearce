...
config_fname = '/home/users/swmclau2/scratch/xi_gg_zheng07_cosmo_v3/config.yaml'
...
#rbins_large.insert(0, rbins_small[-1]) # make sure the middle bin is not cut
    ...
    cov_gg = np.zeros((rbins.shape[0]-1, rbins.shape[0]-1))
...
#for rb, nr in zip([rbins_large], n_rands): #
        # nr removed
        randoms = np.random.random((pos_g.shape[0]*nr,3)) * cat.Lbox / cat.h  # Solution to NaNs: Just fuck me up with randoms
        out = tpcf_subregions(pos_g / cat.h, randoms, rb, sample2 = None, period=cat.Lbox / cat.h,
