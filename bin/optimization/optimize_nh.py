...
fixed_params = {'z':0.0}
hyperparams = {'kernel': (RBF(input_dim=7, ARD=True)+Matern32(input_dim=7, ARD=True)+Bias(input_dim=7),
                          RBF(input_dim=HOD_params, ARD=True) + Matern32(input_dim=HOD_params, ARD=True) +Bias(input_dim=HOD_params) ), \
               'optimize': True}
